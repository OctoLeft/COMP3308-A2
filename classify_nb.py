...
def gaussian_pdf(x, mean, std, epsilon=1e-8):
    exponent = np.exp(-((x - mean) ** 2) / (2 * ((std + epsilon) ** 2)))
    return (1 / ((std + epsilon) * np.sqrt(2 * np.pi))) * exponent
    ...
    for test_sample in X_test:
        ...
        for label in unique_labels:
            ...
            for feature_index, feature_value in enumerate(test_sample):
                ...
                feature_probability += np.log(gaussian_pdf(feature_value, mean, std) + epsilon)
...
result5 = []
for i in range(1,11):
    result1.append(calculate_accuracy(classify_nb(f"tests/train{i}.csv", f"tests/fold{i}.csv"), f"tests/fold{i}.csv"))
    result5.append(calculate_accuracy(classify_nb(f"tests/train{i}.csv", f"tests/fold{i}.csv"), f"tests/fold{i}.csv"))
    
print(f"Accuracy for MyNB: {statistics.mean(result1)}")